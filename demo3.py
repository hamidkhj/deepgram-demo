...
def display_transcription(response, show_confidence=False):
    try:
        ...
        word_count = 0
        for p_idx, p in enumerate(results.channels[0].alternatives[0].paragraphs.paragraphs):
            # Start a new paragraph
            st.markdown(f"<div style='margin-bottom: 25px;'><strong>Paragraph {p_idx+1}</strong></div>", unsafe_allow_html=True)
            
            for s_idx, s in enumerate(p.sentences):
                # Start a new sentence container with improved spacing
                html = '<div class="word-container"><span class="sentence-label">Sentence:</span>'
                ...
                for i, w in enumerate(words_in_sentence):
                    if word_count < len(results.channels[0].alternatives[0].words):
                        ...
                        # Improved padding and spacing with the word-chip class
                        html += f'<span class="word-chip" style="background-color: rgb({r}, {g}, {b}); color: {"white" if confidence < 0.5 else "black"};">{display_text}</span>'
